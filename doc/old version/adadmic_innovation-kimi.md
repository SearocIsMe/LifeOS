# LifeOS 创新点（I1–I6）文献调研

**用途**：为产品规格说明书 v0.6 §8.1「创新点（真正构成壁垒）」提供文献搜索关键词与近一年代表性论文，支撑新颖性判断与论文 Related Work 写作。

**调研日期**：2026-08-05 ｜ 「近一年」口径：约 2025-08 至 2026-08（个别早于窗口但在该方向不可绕过的经典文献单独注明）。

**方法与诚实声明**：

- 所有论文均来自实际检索结果，非凭记忆拼凑；每篇链接均经 FetchURL 逐一抓取验证，确认返回真实论文页面（arXiv 摘要页 / 会议页面）而非 404 或搜索页，方被收录。
- 某方向确属空白时如实标注「Gap」，不凑数——空白本身即是新颖性证据。
- 每方向保留若干已验证的备选论文，供后续扩展阅读。
...
2. [SPASM: Stable Persona-driven Agent Simulation for Multi-turn Dialogue Generation](https://arxiv.org/abs/2604.09212) — arXiv 预印本，2026-04（G. Laban et al.）。不改模型权重解决 persona 漂移：将对话历史存为视角无关的外部表示（Egocentric Context Projection）并逐轮重注入，在三种不同 LLM 骨干（GPT-4o-mini、DeepSeek-V3.2、Qwen-Plus）上验证——即同一 persona schema 的模型无关渲染。
3. [Time, Identity and Consciousness in Language Model Agents](https://arxiv.org/abs/2603.09043) — arXiv 预印本，2026-03（S. Schneider et al.）。形式化脚手架 LM agent 的身份持续性：从脚手架轨迹定义可计算的持久性得分，将记忆/提示架构映射到「身份形态空间」——为「围绕外部化状态组织的 agent 是否仍是同一实例」提供理论语言。（理论性强于实证。）
...
3. [Consistently Simulating Human Personas with Multi-Turn Reinforcement Learning](https://arxiv.org/abs/2511.00222) — NeurIPS 2025，2025-10（M. Abdulhai et al., UC Berkeley / UW / Google）。定义三个自动 persona 漂移指标（prompt-to-line、line-to-line、Q&A 一致性），均经人类标注验证——可直接复用的指标族；其 RL 微调将不一致性降低 >55%。
...
1. [Zep: A Temporal Knowledge Graph Architecture for Agent Memory](https://arxiv.org/abs/2501.13956) — arXiv 预印本，2025-01（Preston Rasmussen et al.）。**注：约 19 个月前发表，早于一年窗口，但它是该方向约 330 次引用的奠基性文献，不可绕过。** Graphiti 双时态知识图谱：跟踪事件发生时间与入库时间，对被取代的边做失效而非覆盖——LifeOS L2 事实需确定性治理的正是这套设计。
...
3. [TOKI: a bitemporal operator algebra for contradiction resolution in LLM-agent persistent memory](https://arxiv.org/abs/2606.06240) — arXiv 预印本，2026-06（Ziming Wang et al.）。最对题的一篇：将四种生产级冲突裁决启发式（last-writer-wins、证据加权合并、等待确认、逐规则策略）形式化为带隔离前置条件的类型化双时态算子，保留 provenance 审计行并证明回放一致性定理——实质上是 I4「确定性时态治理」的形式契约。
...
2. [Agent Memory: Characterization and System Implications of Stateful Long-Horizon Workloads](https://arxiv.org/abs/2606.06448) — arXiv 预印本，2026-06（Yasmine Omri, Ziyu Gan, et al.，含 Alex Pentland、Thierry Tambe）。首个对 agent「跨会话持久存储、检索、更新自身记忆」的系统级刻画；其四轴分类法与读写路径成本分析为持久状态序列化结构提供依据。
...
最贴题的子方向——「生命实例 Schema」与「可携带 agent 身份格式」——是真实空白。检索仅得到协议综述（如 MCP/ACP/A2A/ANP 互操作综述，arXiv 2505.02279，2025）与厂商规范（A2A AgentCard、MCP），它们标准化的是**通信**，而非 **agent 状态/身份序列化**。尚无公认的 2025–2026 论文提出 agent 完整持久身份/记忆状态的可携带标准格式——这是 LifeOS 可占据的白地，上述记忆系统论文提供最近的学术锚点。
...
3. **I5 是白地但学术锚点少**：写作时应引用 agent 记忆系统刻画类论文（I5 三篇）建立语境，再声明通信协议（MCP/A2A）不覆盖状态序列化。
4. **I6 文献充足、空白在执行层**：Related Work 易写，新颖性声明必须精确限定为「可执行、留痕、机器校验的红线策略」，避免与危害分类法文献撞车。
5. **遗留检索项**（后续补充）：I3 的 `"model versioning" provenance replay LLM` 与 ML 血缘方向；I2 的 PersonaScore 纳入对照测量集的实施细节（规格 §8.1 已有建议）。
